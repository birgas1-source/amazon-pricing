    ...
    try {
      ...
      if (alertCount > 0) {
        ...
        if (alertsData.unavailable_ae.length) {
          msg += `🔴 مبقاش متاح في AE (${alertsData.unavailable_ae.length}):\n`
        ...
        if (alertsData.delisted.length) {
          msg += `🟠 مبقاش معروض في الإمارات (${alertsData.delisted.length}):\n`
            ...
            <div className="alerts-header">
              <div className="alerts-summary">
                ...
                <div className="alerts-label">
                  ...
                  {alertFilter === 'unavailable_ae' && '🔴 مبقاش متاح في الإمارات'}
                  {alertFilter === 'delivery_change' && '🚚 تغير في فترة التوصيل'}
                  {alertFilter === 'delisted' && '🟠 مبقاش معروض في الإمارات'}
                </div>
              </div>
            ...
            <div className="alert-subtabs">
              {(() => {
                ...
                const filters = [
                  ...
                  { key: 'price_up', label: 'سعر AE ارتفع', icon: '📈' },
                  { key: 'price_down', label: 'سعر AE نزل', icon: '📉' },
                  { key: 'unavailable_ae', label: 'مبقاش متاح في الإمارات', icon: '🔴' },
                  { key: 'delivery_change', label: 'تغير التوصيل', icon: '🚚' },
                  { key: 'delisted', label: 'مبقاش معروض في الإمارات', icon: '🟠' },
                ]
                return filters
                  ...
                  <button
                    key={f.key}
                    className={`alert-subtab ${alertFilter === f.key ? 'active' : ''}`}
            ...
            <div className="alerts-list">
              {fullList.map(a => {
                ...
                const typeMeta = {
                  ...
                  price_down: { icon: '📉', color: 'success', label: 'سعر AE نزل', action: 'فرصة ربح أعلى' },
                  unavailable: { icon: '⛔', color: 'danger', label: 'مش متاح', action: 'راجع المنتج' },
                  unavailable_ae: { icon: '🚫', color: 'danger', label: 'مش متاح في الإمارات', action: 'المنتج محتاج بديل' },
                  delivery_change: { icon: '🚚', color: 'warning', label: 'فترة التوصيل اتغيرت', action: 'راجع وقت الشحن' },
                  delisted: { icon: '📤', color: 'warning', label: 'مبقاش معروض في الإمارات', action: 'راجع الـ listing' },
          ...
          <div className="sellers-modal" onClick={e => e.stopPropagation()}>
            ...
            {sellersModalData === null ? (
              ...
              <div className="sellers-loading">
                مفيش بيانات بائعين محفوظة — اضغط "تحديث بيانات البائعين" الأول
              </div>
            ) : (
              <>
